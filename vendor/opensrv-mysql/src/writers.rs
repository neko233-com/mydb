...
pub async fn write_err<W: AsyncWrite + Unpin>(
    ...
    write_err_raw(err as u16, err.sqlstate(), msg, w).await
}

pub async fn write_err_raw<W: AsyncWrite + Unpin>(
    error_code: u16,
    sqlstate: &[u8; 5],
    msg: &[u8],
    w: &mut PacketWriter<W>,
    ...
    w.write_u8(0xFF)?;
    w.write_u16::<LittleEndian>(error_code)?;
    w.write_u8(b'#')?;
...
mod tests {
    ...
    use super::write_err_raw;
    use crate::packet_writer::PacketWriter;

    #[tokio::test]
    async fn raw_error_preserves_custom_number_and_sqlstate() {
        let (stream, mut reader) = duplex(128);
        let mut writer = PacketWriter::new(stream);
        write_err_raw(30_001, b"45001", b"mapped", &mut writer)
            .await
            .unwrap();
        let mut packet = [0_u8; 19];
        reader.read_exact(&mut packet).await.unwrap();
        assert_eq!(&packet[4..], b"\xff1u#45001mapped");
    }
}
