...
pub trait AsyncMysqlShim<W: Send> {
    ...
    /// Called when the client resets this connection for reuse.
    ///
    /// Implementations must discard per-connection state while retaining the authenticated
    /// connection itself. A successful callback is acknowledged with an OK packet.
    async fn on_reset_connection(&mut self) -> Result<(), Self::Error> {
        Ok(())
    }
    ...
    /// Returns the client-side file name when this query starts a LOCAL INFILE transfer.
    fn local_infile_name(&self, _query: &[u8]) -> Option<Vec<u8>> {
        None
    }

    /// Maximum accepted LOCAL INFILE payload for one statement.
    fn max_local_infile_size(&self) -> usize {
        1024 * 1024 * 1024
    }

    /// Called after a complete LOCAL INFILE payload has been received.
        ...
        while let Some((seq, packet)) = self.reader.next_async().await? {
            ...
            match res {
                Ok(cmd) => {
                    match cmd.1 {
                        Command::Query(q) => {
                            if let Some(file_name) = self.shim.local_infile_name(q) {
                                if !self
                                    .client_capabilities
                                    .contains(CapabilityFlags::CLIENT_LOCAL_FILES)
                                {
                                    ...
                                    w.error(
                                        ...
                                        b"The client does not permit LOAD DATA LOCAL INFILE",
                                    )
                                    .await?;
                                } else {
                                    ...
                                    let limit = self.shim.max_local_infile_size();
                                    let mut data = Vec::new();
                                    let mut too_large = false;
                                    loop {
                                        let (file_seq, file_packet) =
                                            self.reader.next_async().await?.ok_or_else(|| {
                                                io::Error::new(
                                                    io::ErrorKind::UnexpectedEof,
                                                    "peer terminated LOCAL INFILE transfer",
                                                )
                                            })?;
                                        self.writer.set_seq(file_seq.wrapping_add(1));
                                        if file_packet.is_empty() {
                                            break;
                                        }
                                        if data.len().saturating_add(file_packet.len()) > limit {
                                            too_large = true;
                                        } else if !too_large {
                                            data.extend_from_slice(&file_packet);
                                        }
                                    }
                                    let w = QueryResultWriter::new(
                                        &mut self.writer,
                                    ...
                                    if too_large {
                                        w.error(
                                            ErrorKind::ER_UNKNOWN_ERROR,
                                            b"LOAD DATA LOCAL INFILE exceeds server limit",
                                        )
                                        .await?;
                                    } else {
                                        self.shim.on_local_infile(q, data, w).await?;
                                    }
                                }
                        ...
                        Command::ResetConnection => {
                            self.shim.on_reset_connection().await?;
                            stmts.clear();
                            writers::write_ok_packet(
                                &mut self.writer,
                                self.client_capabilities,
                ...
                Err(_) => {
                    // MySQL reports malformed or unsupported commands but keeps the connection.
                    writers::write_err(
