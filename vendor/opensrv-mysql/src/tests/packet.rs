...
#[test]
fn packet_reader_keeps_multiple_buffered_packets_owned() {
    let data = [
        0x01, 0x00, 0x00, 0x00, 0x10, 0x02, 0x00, 0x00, 0x01, 0x20, 0x21,
    ];
    let mut reader = PacketReader::new(std::io::Cursor::new(data));
    let first = reader.next().unwrap().unwrap().1;
    let second = reader.next().unwrap().unwrap().1;
    assert_eq!(&*first, &[0x10]);
    assert_eq!(&*second, &[0x20, 0x21]);
}
