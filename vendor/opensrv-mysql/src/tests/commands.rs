...
fn it_parses_reset_connection() {
    let (_, command) = parse(&[0x1f]).unwrap();
    assert_eq!(command, Command::ResetConnection);
}

#[test]
fn it_parses_reset_statement() {
    let (_, command) = parse(&[0x1a, 0x2a, 0, 0, 0]).unwrap();
    assert_eq!(command, Command::ResetStatement(42));
}
