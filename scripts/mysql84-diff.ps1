...
$globalRevokeUser = "global_revoke_$([guid]::NewGuid().ToString('N').Substring(0, 8))"
$revokeOptionsUser = "revoke_options_$([guid]::NewGuid().ToString('N').Substring(0, 8))"
$revokeOptionsMissingUser = "missing_revoke_$([guid]::NewGuid().ToString('N').Substring(0, 8))"
$revokeOptionsMissingRole = "missing_role_$([guid]::NewGuid().ToString('N').Substring(0, 8))"
$scopedRevokeTableUser = "scoped_table_$([guid]::NewGuid().ToString('N').Substring(0, 8))"
$scopedRevokeColumnUser = "scoped_column_$([guid]::NewGuid().ToString('N').Substring(0, 8))"
$scopedRevokeRoutineUser = "scoped_routine_$([guid]::NewGuid().ToString('N').Substring(0, 8))"
$scopedRevokeDatabaseUser = "scoped_database_$([guid]::NewGuid().ToString('N').Substring(0, 8))"
$partialRevokesUser = "partial_revokes_$([guid]::NewGuid().ToString('N').Substring(0, 8))"
...
try {
    ...
    $cases = @(
       ...
       [pscustomobject]@{ Name = "json wildcard paths"; Sql = "SELECT JSON_EXTRACT('{`"a`":1,`"b`":2}','`$.*') AS object_wildcard, JSON_EXTRACT('[1,2,3]','`$[*]') AS array_wildcard, JSON_EXTRACT('{`"a`":{`"x`":1},`"b`":{`"x`":2}}','`$.*.x') AS nested_wildcard, JSON_CONTAINS_PATH('{`"a`":1}','one','`$.*') AS path_one, JSON_CONTAINS_PATH('{`"a`":1}','all','`$.*') AS path_all, JSON_LENGTH('{`"a`":[1,2],`"b`": [3]}','`$.*[0]') AS wildcard_length, JSON_VALUE('[1,2]','`$[*]') AS wildcard_value; SELECT JSON_SEARCH('{`"a`":[`"foo`",`"bar`"],`"b`":`"foo`"}','all','foo','\\','`$.*') AS wildcard_search" },
        [pscustomobject]@{ Name = "json recursive paths"; Sql = "SELECT JSON_EXTRACT('{`"a`":1,`"x`":{`"a`":2,`"arr`":[{`"a`":3}]}}','`$**.a') AS recursive_key, JSON_EXTRACT('[1,[2,[3]]]','`$**[0]') AS recursive_index; SELECT JSON_SEARCH('{`"a`":`"root`",`"x`":{`"a`":`"nested`"}}','all','nested','\\','`$**.a') AS recursive_search" },
        [pscustomobject]@{ Name = "json path ranges"; Sql = "SELECT JSON_EXTRACT('[0,1,2,3]','`$[1 to 2]') AS array_range, JSON_EXTRACT('[0,1,2,3]','`$[last]') AS range_last, JSON_EXTRACT('[0,1,2,3]','`$[last-1]') AS range_prior, JSON_EXTRACT('[0,1,2,3]','`$[1 to last]') AS open_range, JSON_LENGTH('[0,1,2,3]','`$[1 to 2]') AS range_length, JSON_CONTAINS_PATH('[0,1,2,3]','one','`$[1 to 2]') AS range_exists, JSON_VALUE('[0,1,2,3]','`$[1 to 2]') AS range_value" },
        [pscustomobject]@{ Name = "json table"; Sql = "SELECT * FROM JSON_TABLE('[{`"id`":1,`"name`":`"a`"},{`"id`":2}]', '`$[*]' COLUMNS (id INT PATH '`$.id', name VARCHAR(10) PATH '`$.name')) AS jt ORDER BY id; SELECT * FROM JSON_TABLE('{`"items`":[{`"id`":1,`"tags`":[`"a`",`"b`"]}]}', '`$.items[*]' COLUMNS (id INT PATH '`$.id', NESTED PATH '`$.tags[*]' COLUMNS (tag VARCHAR(10) PATH '`$'))) AS jt ORDER BY id,tag; SELECT * FROM JSON_TABLE('[{`"id`":1},{`"id`":`"bad`"}]', '`$[*]' COLUMNS (id INT PATH '`$.id' NULL ON ERROR, fallback INT PATH '`$.missing' DEFAULT '7' ON EMPTY, has_id INT EXISTS PATH '`$.id', ord FOR ORDINALITY)) AS jt ORDER BY ord" },
        [pscustomobject]@{ Name = "json table correlated"; Sql = "CREATE TABLE $database.json_table_correlated (id INT PRIMARY KEY, payload TEXT); INSERT INTO $database.json_table_correlated VALUES (1,'[10,20]'),(2,'[30]'),(3,'[]'); SELECT d.id,j.ord,j.value FROM $database.json_table_correlated AS d JOIN JSON_TABLE(d.payload, '`$[*]' COLUMNS (ord FOR ORDINALITY, value INT PATH '`$')) AS j ON TRUE ORDER BY d.id,j.ord; SELECT d.id,j.value FROM $database.json_table_correlated AS d LEFT JOIN JSON_TABLE(d.payload, '`$[*]' COLUMNS (value INT PATH '`$')) AS j ON TRUE ORDER BY d.id,j.value" },
        [pscustomobject]@{ Name = "json aggregates"; Sql = "CREATE TABLE $database.json_aggregate_probe (id INT, value JSON); INSERT INTO $database.json_aggregate_probe VALUES (1,'7'),(2,'`"x`"'),(3,NULL); SELECT JSON_ARRAYAGG(value) AS array_values,JSON_OBJECTAGG(id,value) AS object_values FROM $database.json_aggregate_probe; SELECT JSON_ARRAYAGG(value) AS empty_array,JSON_OBJECTAGG(id,value) AS empty_object FROM $database.json_aggregate_probe WHERE id>99" },
        [pscustomobject]@{ Name = "json aggregate windows"; Sql = "CREATE TABLE $database.json_aggregate_window_probe (id INT, value JSON); INSERT INTO $database.json_aggregate_window_probe VALUES (1,'7'),(2,'`"x`"'),(3,NULL); SELECT id,JSON_ARRAYAGG(value) OVER (ORDER BY id ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_values FROM $database.json_aggregate_window_probe ORDER BY id" },
        ...
        [pscustomobject]@{ Name = "show databases"; Sql = "SHOW DATABASES" },
        ...
        [pscustomobject]@{ Name = "group concat max len"; Sql = "CREATE TABLE $database.group_concat_limit_probe (group_id INT,sort_id INT,value VARCHAR(32)); INSERT INTO $database.group_concat_limit_probe VALUES (1,1,'ab'),(1,2,'cd'),(2,1,'ef'),(2,2,'gh'); SET SESSION group_concat_max_len=4; SELECT group_id,GROUP_CONCAT(value ORDER BY sort_id SEPARATOR '|') AS concat_values FROM $database.group_concat_limit_probe GROUP BY group_id ORDER BY group_id; SHOW WARNINGS; SELECT @@warning_count,@@session.group_concat_max_len; SET SESSION group_concat_max_len=3; SELECT @@session.group_concat_max_len; SET SESSION group_concat_max_len=DEFAULT; SELECT @@session.group_concat_max_len" },
        [pscustomobject]@{ Name = "nested aggregate scalar projection"; Sql = "SELECT CONCAT('x=',COUNT(*)) AS nested_count,COUNT(*)+1 AS plus_count,COALESCE(SUM(1),0) AS nested_sum,HEX(GROUP_CONCAT('😀a','b')) AS nested_group_concat" },
        ...
        [pscustomobject]@{ Name = "regexp functions"; Sql = "SELECT REGEXP_INSTR('abcabc','abc',1,2) AS second_match, REGEXP_INSTR('猫a猫','猫',2,1,1) AS following_position, REGEXP_INSTR('猫a猫','猫',4,1) AS byte_position, REGEXP_SUBSTR('a12 b34','[0-9]+',1,2) AS second_number, REGEXP_SUBSTR('Ab','a',1,1,'i') AS case_match, REGEXP_REPLACE('a1 a2','a[0-9]','x') AS replaced_all, REGEXP_REPLACE('a1 a2','a[0-9]','x',1,2) AS replaced_one, REGEXP_REPLACE('猫a猫','猫','x',4,0) AS byte_replace, REGEXP_REPLACE('a1','a([0-9])','x\\1') AS backref_replace" },
        ...
        [pscustomobject]@{ Name = "multi-expression group concat"; Sql = "CREATE TABLE $database.group_concat_pairs (left_key VARCHAR(8),right_key VARCHAR(8),sort_key INT); INSERT INTO $database.group_concat_pairs VALUES ('a','x',1),('a','x',2),('a','y',3),('b','x',4),('a',NULL,5),(NULL,'z',6); SELECT GROUP_CONCAT(left_key,right_key ORDER BY sort_key SEPARATOR '|') AS pairs,GROUP_CONCAT(DISTINCT left_key,right_key ORDER BY left_key,right_key SEPARATOR '|') AS distinct_pairs FROM $database.group_concat_pairs" },
        ...
        [pscustomobject]@{ Name = "spatial metrics"; Sql = "SELECT ST_GeometryType(ST_LineFromText('LINESTRING(0 0,3 4,6 0)')),ST_Dimension(ST_PolyFromText('POLYGON((0 0,4 0,4 3,0 0))')),ST_NumPoints(ST_LineFromText('LINESTRING(0 0,3 4,6 0)')),ST_AsText(ST_PointN(ST_LineFromText('LINESTRING(0 0,3 4,6 0)'),2)),ST_Length(ST_LineFromText('LINESTRING(0 0,3 4,6 0)')),ST_Area(ST_PolyFromText('POLYGON((0 0,4 0,4 3,0 0))')),ST_NumInteriorRing(ST_PolyFromText('POLYGON((0 0,4 0,4 3,0 0))')),ST_AsText(ST_ExteriorRing(ST_PolyFromText('POLYGON((0 0,4 0,4 3,0 0))'))),ST_Distance(ST_PointFromText('POINT(0 0)'),ST_PointFromText('POINT(3 4)'))" },
        [pscustomobject]@{ Name = "spatial collections"; Sql = "SELECT ST_AsText(ST_MPointFromText('MULTIPOINT((1 2),(3 4))')),ST_GeometryType(ST_MPointFromText('MULTIPOINT((1 2),(3 4))')),ST_Dimension(ST_MPointFromText('MULTIPOINT((1 2),(3 4))')),ST_NumGeometries(ST_MPointFromText('MULTIPOINT((1 2),(3 4))')),ST_AsText(ST_GeometryN(ST_MPointFromText('MULTIPOINT((1 2),(3 4))'),2)); SELECT ST_AsText(ST_MLineFromText('MULTILINESTRING((0 0,3 4),(0 0,0 5))')),ST_Length(ST_MLineFromText('MULTILINESTRING((0 0,3 4),(0 0,0 5))')),ST_Area(ST_MPolyFromText('MULTIPOLYGON(((0 0,4 0,4 3,0 0)),((0 0,2 0,2 2,0 0)))')); SELECT ST_AsText(ST_GeomCollFromText('GEOMETRYCOLLECTION(POINT(1 2),LINESTRING(0 0,3 4))')),ST_NumGeometries(ST_GeomCollFromText('GEOMETRYCOLLECTION(POINT(1 2),LINESTRING(0 0,3 4))')),ST_AsText(ST_GeometryN(ST_GeomCollFromText('GEOMETRYCOLLECTION(POINT(1 2),LINESTRING(0 0,3 4))'),2))" },
        [pscustomobject]@{ Name = "spatial srid and constructors"; Sql = "SELECT ST_AsText(ST_GeomFromText('POINT(1 2)',4326)),ST_SRID(ST_GeomFromText('POINT(1 2)',4326)),ST_AsText(ST_SRID(ST_GeomFromText('POINT(1 2)',4326),3857)),ST_AsText(ST_PointFromText('POINT(1 2)')),ST_AsText(ST_LineFromText('LINESTRING(0 0,1 1)')),ST_AsText(ST_PolyFromText('POLYGON((0 0,1 0,1 1,0 0))'))" },
        [pscustomobject]@{ Name = "spatial predicates"; Sql = "SELECT ST_Equals(ST_PointFromText('POINT(1 2)'),ST_PointFromText('POINT(1 2)')),ST_Contains(ST_PolyFromText('POLYGON((0 0,4 0,4 4,0 4,0 0))'),ST_PointFromText('POINT(1 1)')),ST_Within(ST_PointFromText('POINT(1 1)'),ST_PolyFromText('POLYGON((0 0,4 0,4 4,0 4,0 0))')),ST_Intersects(ST_LineFromText('LINESTRING(0 0,4 4)'),ST_LineFromText('LINESTRING(0 4,4 0)')),ST_Disjoint(ST_PointFromText('POINT(9 9)'),ST_PolyFromText('POLYGON((0 0,4 0,4 4,0 4,0 0))')),ST_Touches(ST_PointFromText('POINT(0 0)'),ST_PolyFromText('POLYGON((0 0,4 0,4 4,0 4,0 0))')),ST_Overlaps(ST_PolyFromText('POLYGON((0 0,4 0,4 3,0 0))'),ST_PolyFromText('POLYGON((1 0,5 0,5 3,1 0))')),ST_Crosses(ST_LineFromText('LINESTRING(0 0,4 4)'),ST_LineFromText('LINESTRING(0 4,4 0)'))" },
        ...
        [pscustomobject]@{ Name = "partial schema revokes"; Sql = "CREATE USER '$partialRevokesUser' IDENTIFIED BY 'partial-password'; SELECT @@GLOBAL.partial_revokes; GRANT SELECT, INSERT, UPDATE ON *.* TO '$partialRevokesUser'; SET GLOBAL partial_revokes=ON; REVOKE INSERT ON $database.* FROM '$partialRevokesUser'; SHOW GRANTS FOR '$partialRevokesUser'; SELECT User_attributes FROM mysql.user WHERE User='$partialRevokesUser'; GRANT INSERT ON $database.* TO '$partialRevokesUser'; REVOKE INSERT ON $database.* FROM '$partialRevokesUser'; REVOKE INSERT ON $database.* FROM '$partialRevokesUser'; REVOKE INSERT ON *.* FROM '$partialRevokesUser'; SET GLOBAL partial_revokes=OFF; DROP USER '$partialRevokesUser'" },
        [pscustomobject]@{ Name = "global revoke without scope"; Sql = "CREATE USER '$globalRevokeUser' IDENTIFIED BY 'global-password'; CREATE TABLE $database.global_revoke_table (id BIGINT PRIMARY KEY); CREATE PROCEDURE $database.global_revoke_proc() SELECT 1; GRANT INSERT ON $database.global_revoke_table TO '$globalRevokeUser'; GRANT EXECUTE ON PROCEDURE $database.global_revoke_proc TO '$globalRevokeUser'; GRANT ALL ON *.* TO '$globalRevokeUser' WITH GRANT OPTION; REVOKE ALL PRIVILEGES, GRANT OPTION FROM '$globalRevokeUser'; SHOW GRANTS FOR '$globalRevokeUser'; DROP USER '$globalRevokeUser'" },
        [pscustomobject]@{ Name = "revoke options and warnings"; Sql = "CREATE USER '$revokeOptionsUser' IDENTIFIED BY 'revoke-options-password'; CREATE TABLE $database.revoke_options (id BIGINT PRIMARY KEY); REVOKE IF EXISTS SELECT ON $database.revoke_options FROM '$revokeOptionsUser'; SHOW WARNINGS; GRANT SELECT ON $database.revoke_options TO '$revokeOptionsUser'; REVOKE IF EXISTS SELECT ON $database.revoke_options FROM '$revokeOptionsUser'; SHOW WARNINGS; REVOKE SELECT ON $database.revoke_options FROM '$revokeOptionsMissingUser' IGNORE UNKNOWN USER; SHOW WARNINGS; REVOKE IF EXISTS $revokeOptionsMissingRole FROM '$revokeOptionsUser' IGNORE UNKNOWN USER; SHOW WARNINGS; REVOKE IF EXISTS ALL PRIVILEGES, GRANT OPTION FROM '$revokeOptionsMissingUser' IGNORE UNKNOWN USER; SHOW WARNINGS; DROP USER '$revokeOptionsUser'" },
        [pscustomobject]@{ Name = "scoped revoke table completeness"; Sql = "CREATE USER '$scopedRevokeTableUser' IDENTIFIED BY 'scoped-table-password'; GRANT SELECT ON $database.accounts TO '$scopedRevokeTableUser'; REVOKE SELECT, INSERT ON $database.accounts FROM '$scopedRevokeTableUser'" },
        [pscustomobject]@{ Name = "scoped revoke column completeness"; Sql = "CREATE USER '$scopedRevokeColumnUser' IDENTIFIED BY 'scoped-column-password'; GRANT SELECT(id) ON $database.accounts TO '$scopedRevokeColumnUser'; REVOKE SELECT(id), UPDATE(id) ON $database.accounts FROM '$scopedRevokeColumnUser'" },
        [pscustomobject]@{ Name = "scoped revoke routine completeness"; Sql = "CREATE PROCEDURE $database.scoped_revoke_probe() SELECT 1; CREATE USER '$scopedRevokeRoutineUser' IDENTIFIED BY 'scoped-routine-password'; GRANT EXECUTE ON PROCEDURE $database.scoped_revoke_probe TO '$scopedRevokeRoutineUser'; REVOKE EXECUTE, ALTER ROUTINE ON PROCEDURE $database.scoped_revoke_probe FROM '$scopedRevokeRoutineUser'" },
        [pscustomobject]@{ Name = "scoped revoke database completeness"; Sql = "CREATE USER '$scopedRevokeDatabaseUser' IDENTIFIED BY 'scoped-database-password'; GRANT SELECT ON $database.* TO '$scopedRevokeDatabaseUser'; REVOKE SELECT, INSERT ON $database.* FROM '$scopedRevokeDatabaseUser'" },
