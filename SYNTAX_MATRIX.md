...
>
> 最后更新：2026-08-12（补齐 `event_scheduler` 全局变量 ON/OFF/DISABLED、DataGrip 探测路径、主键/单列二级索引 next-key/gap 区间锁、RC 记录锁、复合索引全等值点锁和无主键基础隐藏行锁；真实持久 row-id、复杂 JOIN 锁和完整 InnoDB 语义仍按清单逐项验收）。
...
| 锁 IS/IX/S/X、行锁、`FOR UPDATE`/`FOR SHARE`/`LOCK IN SHARE MODE`、NOWAIT/3572、SKIP LOCKED、死锁 1213 | 🟡 Partial | 主键/单列二级索引 next-key/gap、复合索引全等值点锁、RC 实际记录锁、无主键基础隐藏行锁已落地；无主键真实持久 row-id、复杂 JOIN 逐行 SKIP LOCKED、多方环成本化受害者 🔴 Deferred |
