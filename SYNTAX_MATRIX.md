...
| JSON（`JSON_EXTRACT/UNQUOTE/OBJECT/ARRAY/VALID/TYPE/LENGTH/CONTAINS/CONTAINS_PATH/OVERLAPS/SET/REMOVE/ARRAY_APPEND/ARRAY_INSERT/MERGE_PATCH/DEPTH/KEYS/PRETTY/SEARCH/ARRAYAGG/OBJECTAGG`） | 🟡 Partial | 本轮新增 `JSON_ARRAYAGG`/`JSON_OBJECTAGG` 的 SQL NULL、JSON 值、空集合、重复 key 覆盖和窗口累计聚合语义，并纳入 108 项 MySQL 8.4 差分；`JSON_SEARCH` 基础字符串搜索已覆盖；JSON path 仍仅覆盖基础对象键/数组下标，通配路径、完整 JSON schema/搜索优化器语义仍 Deferred |
