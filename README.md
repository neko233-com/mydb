...
|------|------|--------------|--------------|
| 单表写（fsync-per-commit） | 206 ops/s | 70 ops/s | 2.94x |
| 4 actor / 4表 写 P99 延迟 | 21.3 ms | 70.4 ms | 3.30x（低更好） |
| 4 actor / 4表 Group Commit | 321 ops/s | 148 ops/s | 2.17x |
| 读 P50 延迟 | 325 μs | 132 μs | - |
...
- ✅ 本轮同条件持久化基准：单表写 MyDB/MySQL `206/70 ops/s`，4 actor P99 `21.3/70.4 ms`，并发吞吐 `321/148 ops/s`，读 P50 `325/132 μs`；1 次样本、无预热、性能阶段 22.5 秒，原始数据见 [性能报告.md](性能报告.md)
