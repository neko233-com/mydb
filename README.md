...
|------|------|--------------|--------------|
| 单表写（fsync-per-commit） | 212 ops/s | 77 ops/s | 2.77x |
| 4 actor / 4表 写 P99 延迟 | 26.1 ms | 43.1 ms | 1.65x（低更好） |
| 4 actor / 4表 Group Commit | 1148 ops/s | 571 ops/s | 2.01x |
| 读 P50 延迟 | 396 μs | 140 μs | - |
...
- ✅ 本轮同条件持久化基准（v0.1.27、嵌套聚合投影修复后）：单表写 MyDB/MySQL `212/77 ops/s`，4 actor P99 `26.1/43.1 ms`，并发吞吐 `1148/571 ops/s`，读 P50 `396/140 μs`；1 次样本、无预热、性能阶段 15.1 秒，原始数据见 [性能报告.md](性能报告.md)
