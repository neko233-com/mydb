...
|------|------|--------------|--------------|
| 单表写（fsync-per-commit） | 216 ops/s | 80 ops/s | 2.69x |
| 4 actor / 4表 写 P99 延迟 | 29.5 ms | 50.5 ms | 1.71x（低更好） |
| 4 actor / 4表 Group Commit | 329 ops/s | 155 ops/s | 2.13x |
| 读 P50 延迟 | 323 μs | 127 μs | - |
...
- ✅ 本轮同条件持久化基准（索引点查优化后，提交 `8b25ac4`）：单表写 MyDB/MySQL `216/80 ops/s`，4 actor P99 `29.5/50.5 ms`，并发吞吐 `329/155 ops/s`，读 P50 `323/127 μs`；1 次样本、无预热、性能阶段 21.6 秒，原始数据见 [性能报告.md](性能报告.md)
