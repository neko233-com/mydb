...
|------|------|--------------|--------------|
| 单表写（fsync-per-commit） | 203 ops/s | 78 ops/s | 2.59x |
| 4 actor / 4表 写 P99 延迟 | 28.7 ms | 57.3 ms | 2.00x（低更好） |
| 4 actor / 4表 Group Commit | 330 ops/s | 144 ops/s | 2.29x |
| 读 P50 延迟 | 403 μs | 132 μs | - |
...
本版面向单机 MySQL 8.4 常用生产工作负载：3306 提供 MySQL 协议，4306 提供登录保护的 Web SQL IDE/管理 API；新增多列/表达式 `COUNT(DISTINCT ...)` 的 MySQL 元组 NULL 与排序规则语义，并保留 `REGEXP_INSTR`、`REGEXP_SUBSTR`、`REGEXP_REPLACE` 的位置、occurrence、match_type、NULL 和 Unicode 外部行为，以及 `mydb update --check`、空间构造器、JSON、全文、迁移、跨平台更新与失败回滚能力。发布不宣称复制/集群、完整 InnoDB 全部锁边界、完整 GIS/OGC 语义、全部冷门字符集或全部 MySQL 错误码已完成；显式 `LATERAL` 前缀属于额外超集能力，不作为 MySQL 8.4 差分承诺；逐项状态见 [CheckList.md](CheckList.md) 和 [SYNTAX_MATRIX.md](SYNTAX_MATRIX.md)。
...
- ✅ `scripts/mysql84-diff.ps1`：当前源码隔离端口与同机 Docker MySQL 8.4，119/119 差分通过；新增多列/表达式 `COUNT(DISTINCT ...)`，并保留 `REGEXP_INSTR`/`REGEXP_SUBSTR`/`REGEXP_REPLACE` 的 occurrence、return_option、NULL 和 Unicode 位置覆盖，以及 JSON_TABLE、空间、JSON 搜索/路径/聚合、全文相关性/布尔短语/前缀/停止词/查询扩展覆盖
- ✅ 本轮同条件持久化基准（v0.1.24、多列 `COUNT(DISTINCT ...)` 修复后）：单表写 MyDB/MySQL `203/78 ops/s`，4 actor P99 `28.7/57.3 ms`，并发吞吐 `330/144 ops/s`，读 P50 `403/132 μs`；1 次样本、无预热、性能阶段 22.1 秒，原始数据见 [性能报告.md](性能报告.md)
