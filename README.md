...
|------|------|--------------|--------------|
| 单表写（fsync-per-commit） | 216 ops/s | 80 ops/s | 2.70x |
| 4 actor / 4表 写 P99 延迟 | 35.3 ms | 65.4 ms | 1.85x（低更好） |
| 4 actor / 4表 Group Commit | 277 ops/s | 154 ops/s | 1.79x |
| 读 P50 延迟 | 408 μs | 132 μs | - |
...
- ✅ 本轮同条件持久化基准（v0.1.24、多列 `COUNT(DISTINCT ...)` 修复后）：单表写 MyDB/MySQL `216/80 ops/s`，4 actor P99 `35.3/65.4 ms`，并发吞吐 `277/154 ops/s`，读 P50 `408/132 μs`；1 次样本、无预热、性能阶段 22.1 秒，原始数据见 [性能报告.md](性能报告.md)
