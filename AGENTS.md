...
1. **运行自动化基准**：`pwsh -File scripts/bench.ps1`（自动执行 clippy → test → release build → Docker 启动 → bench → 更新 [性能报告.md](性能报告.md)）
...
- 默认 Group Commit 窗口：0μs（自然批量，仅 fsync 期间请求自动聚合；可配置额外等待窗口牺牲延迟换吞吐）
- 数据页大小：16KB (`DEFAULT_PAGE_SIZE`)
- WAL 预分配粒度：8MB
- WAL 文件最大大小：64MB
- Checkpoint 间隔：64 个提交组
